...
RegisterServerEvent("JobGarage:AddData", function(type, vehicle, hash, plate, job)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('JobGarage:RentVehicle', function(paymenttype, finalPrice, vehiclename, vehicle, time, coordsinfo, job, station)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    if job ~= "all" and Config.BanWhenExploit and not HasJob(job) then ExploitBan(src, 'Banned for exploiting') end
    if Player.Functions.GetMoney(paymenttype) >= finalPrice then
        TriggerClientEvent("JobGarage:SpawnRentedVehicle", src, vehicle, vehiclename, finalPrice, time, os.time(), coordsinfo['VehicleSpawn'], paymenttype, job, station)  
...
RegisterServerEvent('JobGarage:BuyVehicle', function(data)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    ...
    if data.paymenttype == "company" and Config.CompanyFunds['Enable'] then
        ...
        if Reciever.PlayerData.job.name == Player.PlayerData.job.name then
            if Reciever.PlayerData.job.grade.level >= data.rank then
                ...
                if account >= data.price then
                    ...
                    TriggerClientEvent("JobGarage:SpawnPurchasedVehicle", data.id, data.vehicle, data.coordsinfo['VehicleSpawn'], data.coordsinfo['CheckRadius'], data.job, data.useownable, data.trunkitems, data.extras, data.liveries, data.station)
                else
        ...
        if Player.Functions.GetMoney(data.paymenttype) >= data.price then
            TriggerClientEvent("JobGarage:SpawnPurchasedVehicle", src, data.vehicle, data.coordsinfo['VehicleSpawn'], data.coordsinfo['CheckRadius'], data.job, data.useownable, data.trunkitems, data.extras, data.liveries, data.station)  
...
RegisterServerEvent('JobGarage:RefundRent', function(paymenttype, refund, clientsource, job)
	local src = source
...
QBCore.Functions.CreateCallback('JobGarage:GetRealTime', function(source, cb)
    cb(os.time())
end)
